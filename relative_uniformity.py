...
def _debug_image(image, all_rect, all_value, local_delta, vec_ru_slop, ring_radius, local_avg, save_path):
    ...
    if False:
        ...
        heat_map_save_path = save_path / (utils.GlobalConfig.get_device_id() + '_RU_HeatMap.png')
    ...
    save_path = save_path / (utils.GlobalConfig.get_device_id() + '_RU.png')
...
def relative_uniformity(src_image, mask_radius, roi_size, delta_angle, border_distance, csv_output, debug_flag=False, save_path=None):
    ...
    if csv_output:
        save_path = save_path / 'ru_data.csv'
        utils.save_dict_to_csv(ru_data, save_path)
